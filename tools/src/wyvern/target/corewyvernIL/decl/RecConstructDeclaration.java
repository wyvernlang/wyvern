...
public class RecConstructDeclaration extends DeclarationWithRHS {
  ...
  /*
   * @Override public BytecodeOuterClass.Declaration emitBytecode() {
   * BytecodeOuterClass.Declaration.VariableDeclaration.Builder vd =
   * BytecodeOuterClass.Declaration.VariableDeclaration.newBuilder()
   * .setDeclarationType(BytecodeOuterClass.VariableDeclarationType.VAL)
   * .setVariable(getName()) .setType(getType().emitBytecodeType())
   * .setInitializer(((Expression) getDefinition()).emitBytecode()); return
   * BytecodeOuterClass.Declaration.newBuilder().setVariableDeclaration(vd).build(
   * ); }
   */
